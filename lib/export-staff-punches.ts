...
function weekdayOf(dateStr: string): string {
  return new Date(dateStr + "T12:00:00").toLocaleDateString("en-AE", { weekday: "long" })
}
...
export function exportStaffPunchesExcel(rows: StaffPunchRow[], month: string, groupBy: "date" | "name" = "date", preOpenedWin?: Window | null) {
  ...
  const monthLabel = new Date(month + "-15").toLocaleDateString("en-AE", { month: "long", year: "numeric" })
...
${sections}
<footer><span>Generated by Ihlamudheen Madrasa · ihlamudheen-madrasa.vercel.app</span><span>${new Date().toLocaleDateString("en-AE")}</span></footer>
<div class="toolbar">
