...
export function exportAttendancePDF(rows: AttendanceRow[], date: string, filterLabel = "All"): { ok: boolean; message: string } {
  ...
  const dateLong = new Date(date + "T00:00:00").toLocaleDateString("en-IN", {
    weekday: "long",
    day: "numeric",
    month: "long",
    year: "numeric",
...
<div class="subtitle">${rows.length} students across ${classCount} class${classCount === 1 ? "" : "es"}${filterNote} · printed ${escHTML(new Date().toLocaleDateString("en-IN"))}</div>
<div class="summary">
...
<footer><span>Generated by Ihlamudheen Madrasa</span><span>${escHTML(new Date().toLocaleDateString("en-IN"))}</span></footer>
...
<div class="subtitle">${rows.length} student${rows.length === 1 ? "" : "s"} across ${classCount} class${classCount === 1 ? "" : "es"}${filterNote} · printed ${escHTML(new Date().toLocaleDateString("en-IN"))}</div>
<div class="summary">
...
<div class="subtitle">${rows.length} class${rows.length === 1 ? "" : "es"} · ${totalTaken} session${totalTaken === 1 ? "" : "s"} conducted${filterNote} · printed ${escHTML(new Date().toLocaleDateString("en-IN"))}</div>
...
export function exportDailyLatePDF(rows: DailyLateRow[], date: string, filterLabel = "All"): { ok: boolean; message: string } {
  const dateLong = new Date(date + "T00:00:00").toLocaleDateString("en-IN", { weekday: "long", day: "numeric", month: "long", year: "numeric" })
...
<div><p>Avg Minutes Late</p><p style="color:#f59e0b;">${avgMin}</p></div>
  ...
  const subtitle = `${rows.length} late student${rows.length === 1 ? "" : "s"} across ${classCount} class${classCount === 1 ? "" : "es"}${filterNote} · printed ${escHTML(new Date().toLocaleDateString("en-IN"))}`
